  ...
  const sidebar = (
    <div className="flex flex-col h-full p-4 gap-2">
      ...
      <div className="flex-1 space-y-1">
        <p className="text-[10px] font-black uppercase tracking-widest text-[#9995A0] px-4 mb-3">{t.workspace}</p>
        {NAV.map(item => <NavItem key={item.to} {...item} onClick={() => setMobileOpen(false)} />)}

        <p className="text-[10px] font-black uppercase tracking-widest text-[#9995A0] px-4 mt-6 mb-3 pt-4">{t.account}</p>
      ...
      <div className="card-clay p-3 mt-2">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-[#635F69] font-medium">{t.freePlan}</p>
          </div>
    ...
    <div className="flex h-screen overflow-hidden bg-[#F4F1FA]">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="absolute start-0 top-0 bottom-0 w-64 bg-white/90 backdrop-blur-xl flex flex-col z-10">
            {sidebar}
          </aside>
        </div>
      )}
