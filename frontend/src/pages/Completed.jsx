...
export default function Completed() {
  ...
  async function handleDelete(id) {
    ...
    try { await api.delete(`/api/tasks/${id}`); setTasks(ts => ts.filter(t => t.id !== id)); toast.success('Deleted'); }
  ...
  return (
    <Layout>
      <div className="p-6 max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-2xl font-black text-[#332F3A]" style={{ fontFamily: 'Nunito, sans-serif' }}>Completed ✅</h1>
          <p className="text-sm text-[#635F69] font-medium mt-0.5">{tasks.length} completed task{tasks.length !== 1 ? 's' : ''}</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="card-clay p-16 text-center">
            ...
            <p className="text-lg font-black text-[#332F3A] mb-1" style={{ fontFamily: 'Nunito, sans-serif' }}>Nothing completed yet</p>
            <p className="text-sm text-[#635F69] font-medium">Tasks you mark as done will appear here</p>
          </div>
        ) : (
