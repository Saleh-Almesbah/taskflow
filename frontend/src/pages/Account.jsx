...
export default function Account() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const high = all.data.filter(tk => tk.priority === 'high' && tk.status !== 'done').length;
  ...
  const statItems = [
    { emoji: '📋', label: t.totalTasksLabel,    value: stats.total,      from: 'from-blue-400',    to: 'to-blue-600' },
    { emoji: '✅', label: t.completedLabel,      value: stats.done,       from: 'from-emerald-400', to: 'to-emerald-600' },
    { emoji: '📈', label: t.completionRateLabel, value: `${completion}%`, from: 'from-violet-400',  to: 'to-violet-600' },
  ];
  ...
  return (
    <Layout>
      <div className="p-6 max-w-2xl mx-auto">
        <div className="mb-8">
          <h1 className="text-2xl font-black text-[#332F3A]" style={{ fontFamily: 'Nunito, sans-serif' }}>{t.accountTitle}</h1>
          <p className="text-sm text-[#635F69] font-medium mt-0.5">{t.accountSubtitle}</p>
        </div>
        ...
        <div className="card-clay p-7 mb-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-sm text-[#635F69] font-medium mt-0.5">{t.memberSince} {joinDate}</p>
        ...
        <div className="card-clay p-7">
          <h2 className="text-lg font-black text-[#332F3A] mb-1" style={{ fontFamily: 'Nunito, sans-serif' }}>{t.changePasswordTitle}</h2>
          <p className="text-sm text-[#635F69] font-medium mb-6">{t.changePasswordSubtitle}</p>
          <form onSubmit={handlePasswordChange} noValidate className="space-y-5">
            <div>
              <label className="label-clay">{t.newPasswordLabel}</label>
              <input type="password" className={`input-clay ${pwErrors.password ? 'ring-4 ring-red-400/30 bg-red-50/50' : ''}`}
                placeholder={t.passwordPlaceholder} value={pwForm.password}
                onChange={e => setPwForm(f => ({ ...f, password: e.target.value }))} />
              {pwErrors.password && <p className="text-red-500 text-xs mt-2 font-medium">{pwErrors.password}</p>}
            </div>
            <div>
              <label className="label-clay">{t.confirmPasswordLabel}</label>
              <input type="password" className={`input-clay ${pwErrors.confirm ? 'ring-4 ring-red-400/30 bg-red-50/50' : ''}`}
                placeholder={t.confirmPlaceholder} value={pwForm.confirm}
            ...
            <button type="submit" className="btn-clay" disabled={pwLoading}>
              {pwLoading ? t.updatingBtn : t.updatePasswordBtn}
            </button>
          </form>
        </div>
      </div>
